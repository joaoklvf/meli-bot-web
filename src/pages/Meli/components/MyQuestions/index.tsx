...
export const MyQuestions = () => {
  const { questions, handleGetOptions, setQuestions } = useQuestions();
  ...
  return (
    <div className="text-white flex flex-col gap-4">
      ...
      {questions?.map(question => (
        <QuestionComponent question={question} key={question.id} setQuestions={setQuestions} questions={questions} />
      ))}
    </div>
  );
}